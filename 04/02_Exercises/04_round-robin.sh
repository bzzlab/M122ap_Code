...
Aufgabenstellung
1. Das Script prüft, ob die Log-Datei app-error.log existiert. Ansonsten
legt es die Log-Datei an.
2. Hat die Log-Datei eine bestimmte Grösse erreicht, dann wird der Extension
einen Zeitstempel angefügt und eine neue, leere, gleichnamige Log-Datei erstellt.
3. Die Grösse einer Datei kann mit folgender Kommando-Folge
(Kommandos getrennt durch Pipe-Symbole) ermittelt werden:
```
du -b file | tr -s '\t' ' ' | cut -d' ' -f1
```
Tipp 1: Wenn Sie genau wissen wollen, was die Kommando-Folge tut,
dann testen Sie diese anhand einer Datei und indem Sie die Kommando-Abschnitte
sukzessive hinzufügen, z.B.
```
du -b file
du -b file | tr -s '\t' ' '
du -b file | tr -s '\t' ' ' | cut -d' ' -f1
